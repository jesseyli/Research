...
# recursive algorithm no good, too many iterations, 12 degrees works but 15 doesnt?
...
def ends(start, img, color):
	...
	while curr_color != color or curr_color == 255:
		if img.getpixel()
		# finish this, trace lines by iterating thru points
	...
	if start_color == color:
		return start
#	if start_color != 1 and start_color != 255:
	if start_color != 255:
...
while i < last_color:
	for j in start[i]:
		e = ends(j,overlay,i+1)
		if e != ():
			angles.append(lawOfCosines(j,e,center))
	i += 1

for j in start[1]:
	overlay.putpixel(j,1)
